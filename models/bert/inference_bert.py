...
def generate_example(question, context):
    def is_whitespace(c):
    ...
    example = SquadExample(qas_id=question, question_text=question, doc_tokens=doc_tokens, orig_answer_text=None,
...
class InputFeatures(object):
    ...
    def __init__(self, unique_id, example_index, doc_span_index, tokens, token_to_orig_map, token_is_max_context,
                 input_ids, input_mask, segment_ids, start_position=None, end_position=None, is_impossible=None):
    ...
    for (example_index, example) in enumerate(examples):
        ...
        for (doc_span_index, doc_span) in enumerate(doc_spans):
            ...
            feature = InputFeatures(unique_id=unique_id, example_index=example_index, doc_span_index=doc_span_index,
                ...
                input_ids=input_ids, input_mask=input_mask, segment_ids=segment_ids, start_position=start_position,
                end_position=end_position, is_impossible=example.is_impossible)
...
def create_model(bert_config, is_training, input_ids, input_mask, segment_ids, use_one_hot_embeddings):
    ...
    model = modeling.BertModel(config=bert_config, is_training=is_training, input_ids=input_ids, input_mask=input_mask,
...
def model_fn_builder(bert_config, init_checkpoint, learning_rate, num_train_steps, num_warmup_steps, use_tpu,
    ...
    def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
        ...
        (start_logits, end_logits) = create_model(bert_config=bert_config, is_training=is_training, input_ids=input_ids,
            input_mask=input_mask, segment_ids=segment_ids, use_one_hot_embeddings=use_one_hot_embeddings)
...
def read_squad_examples(qas):
    ...
    is_impossible = False
    q_id = 1
    for context in qas['context_list']:
        doc_tokens = []
        prev_is_whitespace = True
        for c in context:
            if is_whitespace(c):
        ...
        example = SquadExample(qas_id=str(q_id), question_text=question, doc_tokens=doc_tokens,
            ...
            for start_index in start_indexes:
                for end_index in end_indexes:
                    ...
                    prelim_predictions.append(
                        _PrelimPrediction(feature_index=feature_index, start_index=start_index, end_index=end_index,
                            start_logit=result.start_logits[start_index], end_logit=result.end_logits[end_index]))

        if True:
            prelim_predictions.append(
                _PrelimPrediction(feature_index=min_null_feature_index, start_index=0, end_index=0,
...
class Inference(object):
    ...
    def __init__(self):
        ...
        self.ckpt = 'model_data/model.ckpt-10859'
        #tf.logging.set_verbosity(tf.logging.INFO)
        self.RawResult = collections.namedtuple("RawResult", ["unique_id", "start_logits", "end_logits"])

        self.bert_config = modeling.BertConfig.from_json_file('model_data/bert_config.json')
        #validate_flags_or_throw(bert_config)
        #tf.gfile.MakeDirs()
        self.tokenizer = tokenization.FullTokenizer(vocab_file='model_data/vocab.txt', do_lower_case=True)
        ...
        model_fn = model_fn_builder(bert_config=self.bert_config, init_checkpoint=self.ckpt,
                                    learning_rate=3e-5, num_train_steps=2.0,
                                    num_warmup_steps=None, use_tpu=False,
                                    use_one_hot_embeddings=False)

        # If TPU is not available, this will fall back to normal Estimator on CPU
        # or GPU.
        self.estimator = tf.contrib.tpu.TPUEstimator(use_tpu=False, model_fn=model_fn, config=run_config,
                                                train_batch_size=32,
                                                predict_batch_size=8)

    def response(self,qas):
        eval_examples = read_squad_examples(qas)
        eval_writer = FeatureWriter(filename=os.path.join(self.output_dir, "eval.tf_record"), is_training=False)
        eval_features = []
        def append_feature(feature):
            eval_features.append(feature)
            eval_writer.process_feature(feature)
        convert_examples_to_features(examples=eval_examples, tokenizer=self.tokenizer, max_seq_length=384,
            doc_stride=128, max_query_length=64, is_training=False,
            output_fn=append_feature)
        eval_writer.close()

        predict_input_fn = input_fn_builder(input_file=eval_writer.filename, seq_length=384,
        ...
        for result in self.estimator.predict(predict_input_fn, yield_single_examples=True):
            ...
            all_results.append(self.RawResult(unique_id=unique_id, start_logits=start_logits, end_logits=end_logits))

        res = write_predictions(eval_examples, eval_features, all_results, 3, 30, True)
        return res

def main(_):
    qas = {'question':"where is Stanford?",
           'context_list':["Columbia University is in New York City.",
                           "Stanford is in California.",
                           "Tongji is in Shanghai."]}
    inference = Inference()
    res = inference.response(qas)
    print(res)
